...
from pytorchvideo.layers import distributed as ptv_distributed  # noqa
...
def cat_all_gather(tensor, local=False):
    if not _dist_available():
        return tensor
    world_size = get_local_size() if local else get_world_size()
    if world_size == 1:
        return tensor
...
def get_world_size():
    if hasattr(ptv_distributed, "get_world_size"):
        return ptv_distributed.get_world_size()
    if not _dist_available():
        return 1
    return dist.get_world_size()
...
def _init_distributed_training(num_gpus, shard_id):
    if hasattr(ptv_distributed, "init_distributed_training"):
        return ptv_distributed.init_distributed_training(num_gpus, shard_id)
    return None
